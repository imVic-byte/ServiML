...
export async function onRequestPost(context) {
  ...
  const { filename, fileType, mesOperacion } = body;

  if (!filename || !fileType || !mesOperacion) {
    return new Response(JSON.stringify({ error: "Missing file data" }), { status: 400 });
  }

  // Guardamos en una carpeta "gastos", luego el mes, y luego el archivo
  const key = `gastos/${mesOperacion}/${Date.now()}_${filename.replace(/[^a-zA-Z0-9.-]/g, '_')}`;
