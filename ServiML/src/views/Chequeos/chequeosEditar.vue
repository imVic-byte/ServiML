...
const fotosNuevas = reactive(generarEstructura(() => []))
const fotosExistentes = reactive(generarEstructura(() => []))
const fotosEliminadas = reactive(generarEstructura(() => []))

const seleccionarEstado = (catKey, itemKey, valor) => {
  chequeo[catKey][itemKey] = valor
  if (valor !== 'malo') {
    comentarios[catKey][itemKey] = ''
    fotosNuevas[catKey][itemKey] = []
    // No eliminamos fotosExistentes, se excluirán al guardar porque estado != malo
  }
}

const agregarFotos = (catKey, itemKey, event) => {
  const archivos = Array.from(event.target.files || [])
  fotosNuevas[catKey][itemKey] = [...fotosNuevas[catKey][itemKey], ...archivos]
  event.target.value = ''
}

const eliminarFotoNueva = (catKey, itemKey, index) => {
  fotosNuevas[catKey][itemKey].splice(index, 1)
}

const eliminarFotoExistente = (catKey, itemKey, index) => {
  const url = fotosExistentes[catKey][itemKey].splice(index, 1)[0]
  fotosEliminadas[catKey][itemKey].push(url)
}
...
const obtenerChequeo = async () => {
  try {
    ...
    observacionesGenerales.value = data.observaciones || ''

    // Cargar datos del chequeo en los reactivos
    if (data.datos_chequeo) {
      for (const catKey in categorias) {
        if (data.datos_chequeo[catKey]) {
          for (const item of categorias[catKey].items) {
            const itemData = data.datos_chequeo[catKey][item.key]
            if (itemData) {
              chequeo[catKey][item.key] = itemData.estado || null
              comentarios[catKey][item.key] = itemData.comentario || ''
              fotosExistentes[catKey][item.key] = Array.isArray(itemData.fotos) ? [...itemData.fotos] : []
            }
          }
        }
      }
...
const subirFotosItem = async (catKey, itemKey) => {
  const archivos = fotosNuevas[catKey][itemKey]
  ...
  try {
    ...
    for (const catKey in categorias) {
      ...
      for (const item of categorias[catKey].items) {
        ...
        if (estado === 'malo') {
          // Mantener fotos existentes que no se eliminaron
          const existentes = fotosExistentes[catKey][item.key] || []
          // Subir fotos nuevas
          const nuevasUrls = await subirFotosItem(catKey, item.key)
          fotosFinales = [...existentes, ...nuevasUrls]
        }
    ...
    const { error } = await supabase.from('chequeos').update(payload).eq('id', chequeoId.value)
    if (error) throw error

    modalTitulo.value = 'Chequeo actualizado'
    modalMensaje.value = 'Los cambios se han guardado correctamente.'
    modalExito.value = true
    showModal.value = true
  } catch (error) {
    console.error(error)
    modalTitulo.value = 'Error'
    modalMensaje.value = 'No se pudo actualizar el chequeo. Intenta nuevamente.'
    modalExito.value = false
    showModal.value = true
  } finally {
...
const cerrarModal = () => {
  ...
  if (modalExito.value) {
    router.push({ name: 'ver-chequeo', params: { id: chequeoId.value } })
  }
}
...
<template>
  <div class="servi-white min-h-screen pb-15">
    <navbar titulo="ServiML" subtitulo="Editar Chequeo" class="sticky top-0 z-50" />
      ...
      <div v-if="ot" class="servi-adapt-bg rounded-xl shadow-sm border border-gray-100 mb-6">
        <div class="flex items-center justify-between servi-blue py-6 px-3 rounded-t-xl">
          <h3 class="font-bold text-white text-lg">
            Editar Inspección
            <p class="font-bold text-sm">Chequeo #{{ chequeoId }}</p>
          </h3>
            ...
            <Transition name="slide-malo">
              <div v-if="chequeo[catKey][item.key] === 'malo'" class="mt-3 space-y-3">
                ...
                <div>
                  ...
                  <div v-if="fotosExistentes[catKey][item.key].length > 0 || fotosNuevas[catKey][item.key].length > 0" class="flex flex-wrap gap-2 mb-2">
                    <div v-for="(foto, idx) in fotosExistentes[catKey][item.key]" :key="'ex-'+idx" class="relative group">
                      <img :src="foto" class="h-16 w-16 sm:h-20 sm:w-20 object-cover rounded-lg border-2 border-red-200" />
                      <button @click="eliminarFotoExistente(catKey, item.key, idx)"
                    ...
                    <div v-for="(foto, idx) in fotosNuevas[catKey][item.key]" :key="'nw-'+idx" class="relative group">
                      <img :src="obtenerPreview(foto)" class="h-16 w-16 sm:h-20 sm:w-20 object-cover rounded-lg border-2 border-blue-200" />
                      <div class="absolute bottom-0 left-0 right-0 bg-blue-500 text-white text-[10px] text-center rounded-b-lg font-semibold">Nueva</div>
                      <button @click="eliminarFotoNueva(catKey, item.key, idx)"
                        class="absolute -top-1.5 -right-1.5 bg-red-400 text-white rounded-full w-5 h-5 flex items-center justify-center text-xs font-bold cursor-pointer opacity-0 group-hover:opacity-100 transition-opacity shadow-sm hover:bg-red-600">
                        ✕
                      </button>
                    </div>
