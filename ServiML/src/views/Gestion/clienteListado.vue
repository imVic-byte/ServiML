          ...
          <table class="min-w-full divide-y divide-gray-800 text-sm">
            ...
            <tbody class="servi-adapt-bg divide-y divide-gray-800">
              <tr v-for="cliente in clientesFiltrados" :key="cliente.id" class="hover:opacity-80 transition-colors">
                ...
                <td class="px-6 py-4 whitespace-nowrap servi-grey-font">{{ cliente.email || '—' }}</td>
                <td class="px-6 py-4 whitespace-nowrap servi-grey-font">
                  <span v-if="cliente.telefono">+{{ cliente.codigo_pais || '56' }} {{ cliente.telefono }}</span>
                  <span v-else>—</span>
                </td>
                <td class="px-6 py-4 whitespace-nowrap servi-grey-font">
                  <button @click="verCliente(cliente.id)" class="servi-blue servi-yellow-font px-2 py-1 rounded-full hover:opacity-80 transition-colors">
