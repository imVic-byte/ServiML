...
<template>
  ...
  <div class="bg-gray-50 min-h-screen pb-30">
    <div class="max-w-5xl mx-auto px-4 sm:px-6 pt-4">

      <!-- Header -->
      <div class="mb-6 hidden sm:block">
        <h1 class="text-2xl font-bold text-gray-900">Gestión ServiML</h1>
        <p class="text-sm text-gray-500 mt-1">Administra la configuración general de la empresa.</p>
      </div>
          ...
          <div class="p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
              <div v-for="(campo, key) in {
                ...
                <label class="block">
                  <span class="text-xs text-gray-500 uppercase font-bold tracking-wide">{{ campo }}</span>
                  <input
                    v-if="editandoEmpresa"
                    v-model="empresa[key]"
                    type="text"
                    class="mt-1 block w-full rounded-lg border border-gray-300 px-3 py-2.5 text-sm text-gray-800 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition"
                    :placeholder="campo"
                  />
                  <p v-else class="mt-1 text-gray-800 font-medium text-sm bg-gray-50 rounded-lg px-3 py-2.5">
                    {{ empresa[key] || '—' }}
                  </p>
                </label>
              </div>
            ...
            <div class="mt-6 pt-5 border-t border-gray-100">
              <div class="flex justify-between items-center mb-3">
                <span class="text-xs text-gray-500 uppercase font-bold tracking-wide">Teléfonos de Contacto</span>
              ...
              <div class="space-y-2">
                <div v-for="(tel, index) in empresa.telefonos" :key="'tel-'+index" class="flex items-center gap-2">
                  ...
                  >
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" :class="tel.prioritario ? 'text-yellow-500' : 'text-gray-300'" :fill="tel.prioritario ? 'currentColor' : 'none'" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                  ...
                  <div class="flex-1">
                    <input
                      ...
                      v-model="tel.valor"
                      type="tel"
                      class="block w-full rounded-lg border border-gray-300 px-3 py-2.5 text-sm text-gray-800 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition"
                      placeholder="+56 9 1234 5678"
                    />
                    <p v-else class="text-gray-800 font-medium text-sm bg-gray-50 rounded-lg px-3 py-2.5">
                      {{ tel.valor || '—' }}
                    </p>
                  </div>
                  ...
                  <button v-if="editandoEmpresa && empresa.telefonos.length > 1" @click="eliminarTelefono(index)" class="flex-shrink-0 p-1.5 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-lg transition cursor-pointer">
            ...
            <div class="mt-6 pt-5 border-t border-gray-100">
              <div class="flex justify-between items-center mb-3">
                <span class="text-xs text-gray-500 uppercase font-bold tracking-wide">Emails de Contacto</span>
              ...
              <div class="space-y-2">
                <div v-for="(em, index) in empresa.emails" :key="'em-'+index" class="flex items-center gap-2">
                  ...
                  >
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" :class="em.prioritario ? 'text-yellow-500' : 'text-gray-300'" :fill="em.prioritario ? 'currentColor' : 'none'" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                      <path stroke-linecap="round" stroke-linejoin="round" d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z" />
                    </svg>
                  </button>
                  <div class="flex-1">
                  ...
                  <button v-if="editandoEmpresa && empresa.emails.length > 1" @click="eliminarEmail(index)" class="flex-shrink-0 p-1.5 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-lg transition cursor-pointer">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12" /></svg>
                  </button>
                </div>
              </div>
            ...
            <div v-if="editandoEmpresa" class="flex justify-end gap-3 mt-6 pt-5 border-t border-gray-100">
              <button @click="cancelarEdicionEmpresa" class="px-5 py-2.5 bg-gray-100 text-gray-700 rounded-lg text-sm font-semibold hover:bg-gray-200 transition cursor-pointer">
                Cancelar
              </button>
      ...
      <div v-if="tabActiva === 'talleres'">
        <div class="flex justify-between items-center mb-4">
          <div>
            <h2 class="text-lg font-bold text-gray-900">Talleres Registrados</h2>
            <p class="text-sm text-gray-500">Gestiona los talleres y su capacidad</p>
          </div>
          ...
          <div v-for="taller in talleres" :key="taller.id"
            ...
            <div class="p-5">
              <div class="flex justify-between items-start">
                <div class="flex items-center gap-3">
                  <div class="p-2.5 rounded-full" :class="taller.activo ? 'bg-green-100 text-green-600' : 'bg-gray-100 text-gray-400'">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                      <path stroke-linecap="round" stroke-linejoin="round" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" />
                    </svg>
                  </div>
                  <div>
                    <h3 class="font-bold text-gray-800">{{ taller.nombre }}</h3>
                    <p class="text-sm text-gray-500">{{ taller.direccion || 'Sin dirección' }}</p>
                  </div>
                </div>
                <div class="flex items-center gap-2">
                  <button @click="abrirModalTaller(taller)" class="p-2 text-gray-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition cursor-pointer">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                      <path stroke-linecap="round" stroke-linejoin="round" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                    </svg>
                  </button>
                  <button @click="eliminarTaller(taller.id)" class="p-2 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition cursor-pointer">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
              ...
              <div class="mt-4 flex items-center gap-4">
                <div class="flex-1">
                  <div class="flex justify-between mb-1">
                    <span class="text-xs text-gray-500 font-medium">Capacidad Máxima</span>
                ...
                <span
                  ...
                  :class="taller.activo ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'"
                >
                  <span class="w-1.5 h-1.5 rounded-full mr-1.5" :class="taller.activo ? 'bg-green-400' : 'bg-gray-400'"></span>
        ...
        <div v-if="talleres.length === 0" class="servi-adapt-bg rounded-xl shadow-sm border border-gray-200 p-10 text-center">
          ...
          <p class="text-gray-500 font-medium">No hay talleres registrados</p>
          <p class="text-sm text-gray-400 mt-1">Agrega tu primer taller para comenzar.</p>
        </div>
      </div>
      ...
      <div v-if="tabActiva === 'servicios'">
        <div class="flex justify-between items-center mb-4">
          <div>
            <h2 class="text-lg font-bold text-gray-900">Catálogo de Servicios</h2>
            <p class="text-sm text-gray-500">Define los servicios que ofrece tu taller</p>
          </div>
        ...
        <div class="servi-adapt-bg rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                ...
                <tr v-for="servicio in servicios" :key="servicio.id" class="hover:bg-gray-50 transition-colors">
                  <td class="px-6 py-4 max-w-[200px]">
                    <div class="flex items-center gap-3">
                      ...
                      <span class="text-sm font-medium text-gray-900 truncate">{{ servicio.nombre }}</span>
                    </div>
                  </td>
                  ...
                  <td class="px-6 py-4 whitespace-nowrap text-right">
                    <div class="flex justify-end gap-1">
                      <button @click="abrirModalServicio(servicio)" class="p-2 text-gray-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition cursor-pointer">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                          <path stroke-linecap="round" stroke-linejoin="round" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                        </svg>
                      </button>
                      <button @click="eliminarServicio(servicio.id)" class="p-2 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition cursor-pointer">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                          <path stroke-linecap="round" stroke-linejoin="round" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                        </svg>
                      </button>
          ...
          <div v-if="servicios.length === 0" class="p-10 text-center">
            <p class="text-gray-500 font-medium">No hay servicios registrados</p>
          </div>
        </div>
      </div>
      ...
      <div v-if="tabActiva === 'cuentas'">
        <div class="flex justify-between items-center mb-4">
          <div>
            <h2 class="text-lg font-bold text-gray-900">Cuentas Bancarias</h2>
            <p class="text-sm text-gray-500">Administra las cuentas de la empresa</p>
          </div>
        ...
        <div class="servi-adapt-bg rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                ...
                <tr v-for="cuenta in cuentas" :key="cuenta.id" class="hover:bg-gray-50 transition-colors">
                  <td class="px-6 py-4 whitespace-nowrap">
                    <div class="flex items-center gap-3">
                      ...
                      <span class="font-medium text-gray-900">{{ cuenta.banco }}</span>
                    </div>
                  </td>
                  <td class="px-6 py-4 whitespace-nowrap text-gray-600">{{ cuenta.tipo_cuenta }}</td>
                  <td class="px-6 py-4 whitespace-nowrap font-mono text-gray-800">{{ cuenta.numero_cuenta }}</td>
                  <td class="px-6 py-4 whitespace-nowrap text-gray-600 max-w-[150px] truncate">{{ cuenta.titular }}</td>
                  <td class="px-6 py-4 whitespace-nowrap text-gray-600">{{ cuenta.rut_titular }}</td>
                  <td class="px-6 py-4 whitespace-nowrap text-right">
                    <div class="flex justify-end gap-1">
                      <button @click="abrirModalCuenta(cuenta)" class="p-2 text-gray-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition cursor-pointer">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                          <path stroke-linecap="round" stroke-linejoin="round" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                        </svg>
                      </button>
                      <button @click="eliminarCuenta(cuenta.id)" class="p-2 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition cursor-pointer">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                          <path stroke-linecap="round" stroke-linejoin="round" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                        </svg>
                      </button>
          ...
          <div v-if="cuentas.length === 0" class="p-10 text-center">
            <p class="text-gray-500 font-medium">No hay cuentas bancarias registradas</p>
          </div>
        </div>
      </div>
    ...
    <div v-if="mostrarModalTaller" class="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div class="relative servi-adapt-bg rounded-2xl shadow-2xl w-full max-w-md overflow-hidden">
        ...
        <div class="p-6 space-y-4">
          <div>
            <label class="text-xs text-gray-500 uppercase font-bold">Nombre del Taller</label>
            <input v-model="nuevoTaller.nombre" type="text" class="mt-1 block w-full rounded-lg border border-gray-300 px-3 py-2.5 text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent" placeholder="Ej: Taller Central" />
          </div>
          <div>
            <label class="text-xs text-gray-500 uppercase font-bold">Dirección</label>
            <input v-model="nuevoTaller.direccion" type="text" class="mt-1 block w-full rounded-lg border border-gray-300 px-3 py-2.5 text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent" placeholder="Dirección del taller" />
          </div>
          <div>
            <label class="text-xs text-gray-500 uppercase font-bold">Capacidad Máxima (vehículos)</label>
            <input v-model.number="nuevoTaller.capacidad_maxima" type="number" min="1" class="mt-1 block w-full rounded-lg border border-gray-300 px-3 py-2.5 text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent" />
          </div>
          <div class="flex items-center gap-2">
            <input v-model="nuevoTaller.activo" type="checkbox" id="tallerActivo" class="rounded" />
            <label for="tallerActivo" class="text-sm text-gray-700 font-medium">Taller activo</label>
          </div>
        </div>
        <div class="px-6 pb-6 flex justify-end gap-3">
          <button @click="cerrarModalTaller" class="px-4 py-2.5 bg-gray-100 text-gray-700 rounded-lg text-sm font-semibold hover:bg-gray-200 transition cursor-pointer">Cancelar</button>
  ...
  <Teleport to="body">
    <div v-if="mostrarModalServicio" class="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div class="relative servi-adapt-bg rounded-2xl shadow-2xl w-full max-w-md overflow-hidden">
        ...
        <div class="p-6 space-y-4">
          <div>
            <label class="text-xs text-gray-500 uppercase font-bold">Nombre del Servicio</label>
            <input v-model="nuevoServicio.nombre" type="text" class="mt-1 block w-full rounded-lg border border-gray-300 px-3 py-2.5 text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent" placeholder="Ej: Cambio de Aceite" />
          </div>
          <div>
            <label class="text-xs text-gray-500 uppercase font-bold">Precio (CLP)</label>
            <input v-model.number="nuevoServicio.precio" type="number" min="0" class="mt-1 block w-full rounded-lg border border-gray-300 px-3 py-2.5 text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent" />
          </div>
          <div class="flex items-center gap-2">
            <input v-model="nuevoServicio.activo" type="checkbox" id="servicioActivo" class="rounded" />
            <label for="servicioActivo" class="text-sm text-gray-700 font-medium">Servicio activo</label>
          </div>
        </div>
        <div class="px-6 pb-6 flex justify-end gap-3">
          <button @click="cerrarModalServicio" class="px-4 py-2.5 bg-gray-100 text-gray-700 rounded-lg text-sm font-semibold hover:bg-gray-200 transition cursor-pointer">Cancelar</button>
  ...
  <Teleport to="body">
    <div v-if="mostrarModalCuenta" class="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div class="relative servi-adapt-bg rounded-2xl shadow-2xl w-full max-w-md overflow-hidden">
        ...
        <div class="p-6 space-y-4">
          <div>
            <label class="text-xs text-gray-500 uppercase font-bold">Banco</label>
          ...
          <div>
            <label class="text-xs text-gray-500 uppercase font-bold">Tipo de Cuenta</label>
          ...
          <div>
            <label class="text-xs text-gray-500 uppercase font-bold">Número de Cuenta</label>
          ...
          <div>
            <label class="text-xs text-gray-500 uppercase font-bold">Titular</label>
          ...
          <div>
            <label class="text-xs text-gray-500 uppercase font-bold">RUT Titular</label>
        ...
        <div class="px-6 pb-6 flex justify-end gap-3">
          <button @click="cerrarModalCuenta" class="px-4 py-2.5 bg-gray-100 text-gray-700 rounded-lg text-sm font-semibold hover:bg-gray-200 transition cursor-pointer">Cancelar</button>
